...
class InpaintingTrainDataset(Dataset):
    def __init__(self, indir, mask_generator, transform):
        in_files_jpg = list(glob.glob(os.path.join(indir, '**', '*.jpg'), recursive=True))
        in_files_png = list(glob.glob(os.path.join(indir, '**', '*.png'), recursive=True))
        self.in_files = in_files_jpg + in_files_png
